...
| `messages` | JSON 数组 | 原始、有序的 Chat Completions `messages`、Responses `input` 或 Anthropic Messages 结构，保留角色、内容块、工具调用和可保留的未知消息字段。Anthropic 顶层 `system` 作为首个 `{"role":"system","content":...}` 条目；仅当 `messages_status` 为 `available` 时出现 |
| `messages_status` | string | 可提取的 Chat Completions、含明确角色的 Responses input 或 Anthropic Messages 为 `available`；没有可确定角色的端点或 Responses input，以及含未知角色的 Anthropic 条目为 `raw_only`；网关无法重读已验证请求体时为 `unreadable` |
...
对于 Chat Completions，网关在使用 `CombineText` 或转换协议前提取原生请求 `messages` 数组。对于 Responses，网关在 `ParseInput()` 前提取原始 `input` 数组；只要数组含有明确角色，就完整保留该数组，包括其中无角色的工具输出和非文本项。对于 Anthropic Messages，网关把顶层 `system` 作为首个 `system` 角色条目，再按顺序保留原生 `messages` 的角色与内容结构。三种协议均保留消息顺序和 JSON 结构，不从扁平文本推断角色。没有可验证角色消息模型的端点、没有可确定角色的 Responses input，或含未知角色的 Anthropic 条目，发送 `messages_status=raw_only`，不会把 Prompt 猜作用户轮次。
