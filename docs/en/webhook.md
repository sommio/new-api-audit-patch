...
| `messages` | JSON array | Original ordered Chat Completions `messages`, Responses and `/v1/responses/compact` `input`, Anthropic Messages structure, or Gemini `contents`. All retain roles, content blocks, tool calls, and preserved unknown message fields. Anthropic and Gemini top-level system instructions are first `{"role":"system","content":...}` entries. Omitted unless `messages_status` is `available` |
| `messages_status` | string | `available` for extractable Chat Completions, role-bearing Responses or `/v1/responses/compact` input, Anthropic Messages, or Gemini GenerateContent requests with explicit `user` or `model` roles; `raw_only` for endpoints without a role-message model, Responses input without determinable roles, Anthropic entries with an unknown role, and Gemini requests with absent or unknown content roles; or `unreadable` when the gateway cannot re-read the validated request body |
...
For Chat Completions, the gateway extracts the native request `messages` array before using `CombineText` or converting protocols. For Responses and `/v1/responses/compact`, it extracts the original `input` array before `ParseInput()` and preserves the complete array when it contains an explicit role, including unassigned tool-output and non-text items alongside it. For Anthropic Messages, it emits top-level `system` as the first `system` role entry, then preserves native `messages` roles and content structures in order. For Gemini GenerateContent, it emits `systemInstruction` (or accepted `system_instruction`) as the first `system` role entry, then preserves the native `contents` entries in order only when every entry has an explicit `user` or `model` role. These protocols preserve message order and JSON structure rather than deriving roles from flattened text. Endpoints and Responses input without a verifiable role-message model, Anthropic entries with an unknown role, and Gemini requests with absent or unknown content roles send `messages_status=raw_only`; they do not infer a user turn from a Prompt.
